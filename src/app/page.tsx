...
<Image priority={true} src={heroImg} alt={"Hero Image"} className={`size-full filter brightness-[40%]`} />
  ...
  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 flex flex-col items-center lg:flex-row justify-between w-[90%] gap-10">
    <motion.div
      animate={{ x: 0, opacity: 1 }}
      initial={{ opacity: 0, x: -100 }}
      transition={{ delay: 0.2, duration: 0.8 }}
      className="lg:w-1/2 mx-auto grid grid-cols-1 gap-2 text-white"
    >
      <h1 className="flex flex-col gap-2 text-center pt-4 lg:pt-0">
        <span className="text-2xl font-semibold">
          Welcome to
        </span>
        <span className="text-3xl lg:text-[3rem] font-semibold lg:leading-[1.1]">
          {schoolName}
        </span>
      </h1>

      <div className="mb-4">
        <p className="text-[#f0f0f0] italic text-center">
          ...Building God-fearing leaders of tomorrow
          {/* Welcome to Faith Baptist Nursery and Primary School. This platform is
          designed to be user-friendly for parents, pupils, and visitors. We
          hope it provides you with valuable insights into{" "}
          <span className="font-[500]">
            our school’s mission: offering pupils an excellent, all-round
            education with strong moral values.
          </span> */}
        </p>
      </div>
      {/* <h6 className="text-white font-bold italic">
        Admission is ongoing, hurry to school now to apply!!!
      </h6> */}
    </motion.div>
  </div>
</div>
     ...
     Read more
      ...
      <AnimatedContainer className="text-slate-700 bg-blue-50">
        <Space />
        <Space border="" />
        ...
        <Space border="" />
          ...
          >
            ...
            >
              ...
              <Image src={headTeacher} className="size-full md:mr-auto rounded-md" alt="Head Teacher" />
            </motion.div>
          </aside>
